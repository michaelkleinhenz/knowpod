...
// summarize) and answers questions. Runs on core 0 at low priority so the UI
// and audio capture stay responsive. Temporary failures are retried with
// backoff; permanent ones put the recording into the "error" state.

void worker_begin();
void worker_kick();                     // new work may be available
void worker_set_paused(bool paused);    // while recording (Wi-Fi is off then)

void worker_retry(const String &id);                          // resume an errored recording
